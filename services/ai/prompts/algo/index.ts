...
export const getLessonPlanSystemInstruction = (
    ...
    **DYNAMIC REVIEW PROTOCOL (CRITICAL)**:
    - I have provided a list of **RECENT MISTAKES** in the user prompt.
    - **TASK**: For at least 3 screens, you MUST pick a specific mistake from that list (if available), identify the underlying logic trap, and generate a **NEW interactive widget** (Fill-in or Parsons) that tests that specific logic.
    - **RULE**: Do NOT just copy the old question. Create a VARIATION. 
      - Example: If they failed "Loop Boundary", generate a new loop with different start/end points but same boundary logic.
    - **LABEL**: If you do this, add a 'callout' widget before the question saying: "Review: You missed this logic previously."
        `;
    }
