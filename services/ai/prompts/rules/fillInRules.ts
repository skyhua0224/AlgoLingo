...
- **FILL-IN WIDGET CONSTRAINTS**:
    ...
    - **NO DUPLICATES**: The 'options' array MUST NOT contain duplicate strings. If the answer is 'num', do not include 'num' twice.
    ...
    - **CRITICAL - QUALITY CONTROL**:
        - **NO AMBIGUOUS VARIABLES**: Do **NOT** create blanks for variable NAMES on the left side of an assignment UNLESS that variable was already defined in the visible code above.
          - ❌ BAD: \`__BLANK__ = 0\` (Could be count, total, res, sum...)
          - ✅ GOOD: \`count = __BLANK__\` (Test the initialization value)
          - ✅ GOOD: \`for __BLANK__ in nums:\` (Only if 'num' or 'x' is standard convention, but verify context). Better: \`for i, num in __BLANK__(nums):\`
