...
import { Brain, Cpu, Sparkles, Swords, User, Settings, RotateCcw } from 'lucide-react';
...
export const MobileNav: React.FC<MobileNavProps> = ({ activeTab, onTabChange, onOpenSettings, t }) => {
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-[#0c0c0c]/90 backdrop-blur-lg border-t border-gray-200 dark:border-white/10 flex justify-around py-3 pb-6 z-50 shadow-2xl overflow-x-auto">
      <MobileNavItem 
        icon={<Brain size={22} />} 
        label="Algo" 
...
const MobileNavItem = ({ icon, label, active, onClick }: any) => (
  <button 
    onClick={onClick}
    className={`flex flex-col items-center gap-1 min-w-[50px] p-1 rounded-xl transition-all duration-300 flex-shrink-0
      ${active 
