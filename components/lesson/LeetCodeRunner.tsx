...
export const LeetCodeRunner: React.FC<LeetCodeRunnerProps> = ({ plan, preferences, language, onSuccess }) => {
    ...
    // Retrieve pre-calculated context from cache using problem ID (activeProblem must exist)
    const problemId = state.activeProblem?.id;
    const data = problemId ? state.problemDataCache[problemId]?.context : null;

    if (!data) return <div className="p-8 text-center text-red-500">Error: Problem context not found. Please reload.</div>;

    const activeSolution = plan.context?.targetSolution;

    // Use specific strategy data if available, else fallback to global context
    const sidebarCode = activeSolution ? {
        ...
        caption: activeSolution.title
    } : data.sidebar.codeSolution;

    // Use specific concept if available
    const conceptData = activeSolution?.concept || data.sidebar.concept;

    return (
        <div className="flex h-full relative bg-gray-50 dark:bg-dark-bg overflow-hidden">
            <GlobalAiAssistant problemName={plan.title} preferences={preferences} language={language} currentPlan={plan} />
                ...
                <div className="p-4 space-y-4 min-w-[300px]">
                     <div className="mb-2">
                        <p className="text-xs text-gray-400 font-bold uppercase tracking-wider text-center">
                            {language === 'Chinese' ? "助手面板" : "Assistant Panel"}
                        </p>
                    </div>
                    
                    {conceptData && (
                        <CollapsibleSection title={<><Lightbulb size={14}/> {activeSolution ? "Core Concept" : "Concept Card"}</>} defaultOpen={true}>
                            <FlipCardWidget widget={{
                                id: 'lc-concept',
                                type: 'flipcard',
                                flipcard: {
                                    front: conceptData.front,
                                    back: conceptData.back,
                                    mode: 'learn'
                                }
                            }} language={language} />
                        </CollapsibleSection>
                    ...
                    <CollapsibleSection title={<><Code size={14}/> {activeSolution ? activeSolution.title : "Reference Solution"}</>} defaultOpen={true}>
                        <InteractiveCodeWidget widget={{
                            id: 'lc-code',
                            type: 'interactive-code',
                            interactiveCode: sidebarCode
                        }} language={language} />
                    </CollapsibleSection>
                </div>
            </div>
